...
class WardrobeButton extends AbstractButton {
    ...
    @Override protected void renderContents(GuiGraphics g,int mx,int my,float delta) {
        ...
        g.drawCenteredString(font,label,x+w/2,y+(h-8)/2,active ? (chosen ? 0xFFFFF6E6 : TEXT) : 0xFF77736D);
    }
    static void panel(GuiGraphics g,int x,int y,int w,int h,int border,int fill) {
