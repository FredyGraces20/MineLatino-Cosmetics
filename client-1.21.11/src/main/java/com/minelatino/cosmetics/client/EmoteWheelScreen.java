...
public final class EmoteWheelScreen extends Screen {
    ...
    private String keyName(){return InputConstants.getKey(new KeyEvent(keys.key(),-1,0)).getDisplayName().getString();}
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event,boolean doubleClick){double mouseX=event.x(),mouseY=event.y();if(event.button()==0&&avatar!=null){int cx=width/2,cy=height/2-5;for(int i=0;i<clips.size();i++){double a=Math.PI*2*i/clips.size()-Math.PI/2;int x=cx+(int)(Math.cos(a)*Math.min(120,width/4)),y=cy+(int)(Math.sin(a)*Math.min(75,height/4));if(Math.abs(mouseX-x)<46&&Math.abs(mouseY-y)<10){String clip=clips.get(i);CosmeticsClient.instance().playEmote(clip,avatar.animations().length(clip));onClose();return true;}}}return super.mouseClicked(event,doubleClick);}
    @Override public boolean keyPressed(KeyEvent event){if(capturing&&event.key()!=256){keys.set(event.key());capturing=false;rebuildWidgets();return true;}return super.keyPressed(event);}
