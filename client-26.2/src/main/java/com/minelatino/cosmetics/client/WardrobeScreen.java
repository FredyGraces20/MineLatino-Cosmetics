...
public final class WardrobeScreen extends Screen {
        ...
        for(int n=page*rows;n<Math.min(filtered.size(),(page+1)*rows);n++) {
            ...
            var card=new WardrobeButton(collectionX+8,y,collectionW-16,34,item.name(),()->{
                ...
                @Override protected void extractContents(GuiGraphicsExtractor g,int mx,int my,float delta) {
                    ...
                    g.text(font,font.plainSubstrByWidth(item.name(),Math.max(8,getWidth()-39)),getX()+35,getY()+5,TEXT,false);
                    boolean saved=item.id().equals(controller.snapshot().equipped().get(item.slot()));
                    String label=saved ? "Equipado · "+CosmeticSlot.label(item.slot()) : CosmeticSlot.label(item.slot());
                    g.text(font,font.plainSubstrByWidth(label,Math.max(8,getWidth()-39)),getX()+35,getY()+20,saved ? ACCENT : DIM,false);
                }
            };
    ...
    @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float delta) {
        g.text(font,width>=440 ? "MINELATINO / ARMARIO" : "ML / ARMARIO",10,16,WardrobeButton.ACCENT,false);
        ...
        g.text(font,font.plainSubstrByWidth(previewTitle,Math.max(1,previewW-70)),previewX+8,top+8,WardrobeButton.DIM,false);
        g.text(font,font.plainSubstrByWidth("COLECCIÓN "+(page+1)+"/"+pages,collectionW-56),collectionX+8,top+6,WardrobeButton.DIM,false);
        ...
        } else g.textWithWordWrap(font,Component.literal("Entra a un mundo para ver tu skin en 3D."),previewX+10,top+45,previewW-20,WardrobeButton.DIM);
        if(cards.isEmpty()) {
            ...
            g.textWithWordWrap(font,Component.literal(empty),collectionX+12,top+47,collectionW-24,WardrobeButton.DIM);
        }
        String detail=item==null ? "Elige uno para probarlo en 3D" : item.name();
        g.text(font,font.plainSubstrByWidth(detail,collectionW-16),collectionX+8,bottom-37,WardrobeButton.DIM,false);
        ...
        g.text(font,font.plainSubstrByWidth(message==null ? "" : message,width-16),8,height-17,
    ...
    private void drawModelBadge(GuiGraphicsExtractor g,int x,int y,String slot,boolean ready,boolean failed) {
        ...
        g.centeredText(font,symbol,x+13,y+7,WardrobeButton.TEXT);
