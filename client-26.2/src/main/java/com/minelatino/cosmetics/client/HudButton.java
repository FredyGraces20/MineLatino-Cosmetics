...
final class HudButton extends AbstractButton {
    ...
    @Override protected void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float delta) {
        ...
        graphics.centeredText(font, text, getX() + getWidth() / 2, getY() + (getHeight() - 8) / 2,
                active ? (chosen ? 0xFFA8F3FF : 0xFFF2F7FA) : 0xFF707A82);
    }

    static void panel(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int border, int fill) {
