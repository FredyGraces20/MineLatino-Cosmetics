...
public final class HudOverlay {
    ...
    public static void render(GuiGraphicsExtractor graphics, boolean editor) {
        Minecraft minecraft = Minecraft.getInstance();
        if (!editor && (minecraft.gui.hud.isHidden() || minecraft.player == null || minecraft.gui.screen() != null)) return;
    ...
    private static Bounds renderWidget(GuiGraphicsExtractor graphics, String id, HudConfig.Widget widget, boolean editor) {
        Minecraft minecraft = Minecraft.getInstance();
        ...
        if (id.equals(HudConfig.ARMOR)) {
            ...
            for (String line : lines) {
                graphics.text(font, line, 8, lineY, widget.enabled() ? TEXT : MUTED, false);
                lineY += 11;
            }
        }
    ...
    private static void renderArmor(GuiGraphicsExtractor graphics, int x, int y, HudConfig.Widget widget, boolean editor) {
        Minecraft minecraft = Minecraft.getInstance();
        Font font = minecraft.font;
        ...
        for (int i = 0; i < slots.length; i++) {
            ...
            if (!stack.isEmpty()) {
                ...
                graphics.text(font, label, percentX,
                        vertical ? itemY + 4 : y + 26, durabilityColor(percent), false);
            } else {
                graphics.text(font, "-", itemX + 5, itemY + 5, MUTED, false);
            }
        }
        if (editor && !HudConfig.get(minecraft.gameDirectory.toPath()).widget(HudConfig.ARMOR).enabled()) {
            graphics.text(font, "apagado", x + (vertical ? 3 : 54), y + (vertical ? 81 : 27), MUTED, false);
        }
    }

    private static void renderCompass(GuiGraphicsExtractor graphics, int x, int y) {
        Minecraft minecraft = Minecraft.getInstance();
        ...
        for (int angle = 0; angle < 360; angle += 15) {
            ...
            if (major) graphics.centeredText(minecraft.font, cardinal(angle), tickX, y + 20, TEXT);
            else graphics.centeredText(minecraft.font, Integer.toString(angle), tickX, y + 21, 0xFF7F8B95);
        }
        graphics.disableScissor();
        graphics.fill(center, y + 3, center + 1, y + 8, BORDER);
        graphics.centeredText(minecraft.font, Math.round(heading) + "°", center, y + 29, 0xFF8DEBFF);
    }
    ...
    private static void renderInput(GuiGraphicsExtractor graphics, int x, int y) {
        Minecraft minecraft = Minecraft.getInstance();
    ...
    private static void key(GuiGraphicsExtractor graphics, int x, int y, int width, int height, String label, boolean down) {
        rounded(graphics, x, y, width, height, down ? 0xD020D9FF : 0x90404750);
        graphics.centeredText(Minecraft.getInstance().font, label, x + width / 2, y + 5,
                down ? 0xFF071217 : TEXT);
    }

    private static void panel(GuiGraphicsExtractor graphics, int x, int y, int width, int height, HudConfig.Widget widget) {
    ...
    private static void rounded(GuiGraphicsExtractor graphics, int x, int y, int width, int height, int color) {
