        ...
        for(int n=page*rows;n<Math.min(filtered.size(),(page+1)*rows);n++) {
            ...
            var card=new WardrobeButton(collectionX+8,y,collectionW-16,34,item.name(),()->{
                ...
                @Override protected void renderWidget(GuiGraphics g,int mx,int my,float delta) {
                    ...
                    // Model PNGs are UV atlases, not product thumbnails. Loading remains
                    // asynchronous, while the card uses a bounded badge and the actual
                    // textured geometry is rendered only by the 3D preview on the left.
